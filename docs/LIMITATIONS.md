...
| **Fan speed cannot be set** | `SFNS` is gated on an `MFGM` flag no AML path ever sets; the DPTF `TFN1` cooling device accepts writes the EC ignores; `F0PD`/`F1PD` are read-only from AML; and `\_SB.PC00.LPCB.H_EC.WTER`, the method that *does* drive fans on a MagicBook Art 14, does not exist in this firmware. All tested. Which *curve* the EC follows can be selected, through `\IFCI`, and that is measured but not shipped — see [`patch/fan/README.md`](../patch/fan/README.md#choosing-the-ecs-fan-curve-yes-and-it-is-not-shipped) |
...
| **Keyboard backlight keys do nothing** | the keys arrive, `huawei-wmi` even creates `huawei::kbd_backlight`, and every write to it fails with `-ENODEV`. **Not a firmware limitation:** the DSDT exposes an 8-bit EC field `KBBL` at offset `0x41` with `\GKBM` / `\SKBM` around it, three levels, readable and writable, and a GPL driver for the sibling FMB-P already implements exactly that mapping. It needs a DMI entry and a tester, not research. See [the ZQC-P page](hardware/zqc-p.md#the-keyboard-backlight-is-reachable) |
